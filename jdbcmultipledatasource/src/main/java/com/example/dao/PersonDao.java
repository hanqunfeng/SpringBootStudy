...
public class PersonDao {
    ...
    public int savePerson2(Person person){
        String sql = "INSERT INTO `springboot1`.`person` (`p_name`, `p_age`) VALUES (?, ?)";
        int result = jdbcTemplate2.update(sql,new Object[]{person.getName(),person.getAge()});
        return result;
    }

    public List<Person> getAllPersonList2(){
        String sql = "select * from person s";
        List<Person> list = jdbcTemplate2.query(sql,new PersonMapper());
        return list;
    }
